...
struct InferenceEngineInner {
    /// ONNX Runtime session – Mutex (not RwLock) because Session::run() requires &mut self
...
impl InferenceEngine {
                ...
                session_builder = match session_builder.with_execution_providers([
                    ...
                    Err(e) => {
                        ...
                        Session::builder()
                            ...
                            .with_intra_threads(config.num_threads)
                            .map_err(|e2| DetectorError::ModelLoadError(format!("Failed to set thread count: {}", e2)))?
                    }
                };
            }
            #[cfg(not(feature = "gpu"))]
    ...
    pub fn run_inference(
        ...
        // Preprocess – returns Result, propagate errors instead of panicking
    ...
    fn run_session(&self, input: &Array4<f32>) -> DetectorResult<Vec<f32>> {
        ...
        // Mutex provides &mut Session which Session::run() requires
        ...
        Ok(tensor_data.1.iter().copied().collect())
    }
